...
Rein deskriptiv lässt sich bereits feststellen, dass Männer mit `round(mean(data$Weight.loss[data$Gender == "Males"]), 2)`kg etwas weniger abgenommen zu haben scheinen, als Frauen mit `round(mean(data$Weight.loss[data$Gender == "Males"]), 2)`kg. 
...
Rein deskriptiv lässt sich bereits feststellen, dass das Gewicht zur 1. Messung mit `round(mean(data$gewicht_prä), 2)`kg etwas höher zu sein scheint, als zur 2. Messung mit `round(mean(data$gewicht_post), 2)`kg. 
...
Nehmen wir dafür einmal an, unsere Therapieerfolg Varianzte hätte eine 3. Ausprägung "Rückfall". Das wären Personen, die zunächst eine Remission hatten, dann aber wieder erkranken.
